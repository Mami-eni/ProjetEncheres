...
public class EnchereJDBC implements EnchereDAO {
	...
	public void insert(Enchere ench) {
		try(Connection cx = Connect.getConnection()){
			...
			request.setInt(3, ench.getArt().getNoArticle());
			request.setInt(4, ench.getUtil().getNoUtilisateur());
	...
	public Enchere enchereBuilder(ResultSet rs) {
		...
		try {
			...
			ench.setArt(art.selectByNo(rs.getInt("no_article")));
			ench.setUtil(util.selectByNo(rs.getInt("no_utilisateur")));	
		}catch(Exception e) {
			System.out.println(e.getMessage());
		}
		return ench;
